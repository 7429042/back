...
export class ProgramsWriteService {
  ...
  private async ensureUniqueSlug(base: string, executedId: Types.ObjectId) {
    ...
    while (
      await this.repo.exists(
        executedId ? ({ slug: candidate, _id: { $ne: executedId } } as any) : ({ slug: candidate } as any),
      )
      ) {
      candidate = `${base}-${i++}`;
    }
  ...
  async publish(id: string): Promise<ProgramResponseDto> {
    ...
    if (doc.status === 'published') throw new BadRequestException('Program is already published');
    ...
    if (!doc.title || !doc.title.trim()) {
      ...
      if (titleTrim.length < 3) errors.push('Title must be at least 3 characters');
      const baseSlug = normalizeSlug(titleTrim);
      const duplicate = await this.repo.exists({ slug: baseSlug, status: 'published', _id: { $ne: doc._id } } as any);
      if (duplicate) errors.push('Program with the same title is already published');
    }
    if (!doc.category) errors.push('Category is required');
    if (typeof doc.hours !== 'number' || doc.hours <= 0) errors.push('Hours must be a positive number');
    if (errors.length) throw new BadRequestException(`Cannot publish program: ${errors.join(', ')}`);
    ...
    const view = (await this.repo.findByIdLean(doc._id.toString())) as AnyProgram | null;
    if (!view) throw new NotFoundException('Program not found');
    return mapProgram(view);
  }
  async updateDraft(
    id: string,
    updates: Partial<Pick<Program, 'title' | 'description' | 'hours' | 'category' | 'price'>>,
  ): Promise<ProgramResponseDto> {
    if (!Types.ObjectId.isValid(id)) throw new NotFoundException('Program not found');

    const doc = await this.repo.findById(id).exec();
    if (!doc) throw new NotFoundException('Program not found');
    if (doc.status !== 'draft') throw new BadRequestException('Program is not in draft status');
    ...
    if (typeof updates.description !== 'undefined') doc.description = updates.description as any;
    if (typeof updates.hours !== 'undefined') doc.hours = updates.hours as any;
    if (typeof updates.category !== 'undefined') doc.category = updates.category as any;
    if (typeof (updates as any).price !== 'undefined') (doc as any).price = (updates as any).price;
